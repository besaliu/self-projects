...
int main(){
   ...
   printf("%s\n", equals(A, A)?"true":"false" );
     // Test case to check matrix multiplication with identity matrix
   Matrix I = newMatrix(n);
   for (int i = 1; i <= n; i++) {
       changeEntry(I, i, i, 1.0); // Fill the diagonal of the matrix with 1.0
   }

   // Test matrix multiplication A * I = A (where I is the identity matrix)
