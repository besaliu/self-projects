...
def full(board):
    for row in board:
        for col in row:  # Corrected the loop variable
            if not col:
                return False
    return True
...
def reset_game():
    global board, yourscore, cpuscore, selected, p1
    board = [[[] for _ in range(3)] for _ in range(3)]
    yourscore = 0
    cpuscore = 0
    selected = False
...
while True:
            ...
            if event.button == 1 and selected:
                ...
                if 0 <= clicked_row < 3 and 0 <= clicked_col < 3:
                    if not board[clicked_row][clicked_col]:
                        ...
                        if top:
                            if can_capture(snake_image, snake_images[top[0]], 'top') and board[clicked_row][
                                clicked_col][1] != top[1]:
                            ...
                            if can_capture(snake_image, snake_images[bottom[0]], 'bottom') and board[clicked_row][
                                clicked_col][1] != bottom[1]:
                        ...
                        if left:
                            if can_capture(snake_image, snake_images[left[0]], 'left') and board[clicked_row][
                                clicked_col][1] != left[1]:
                            ...
                            if can_capture(snake_image, snake_images[right[0]], 'right') and board[clicked_row][
                                clicked_col][1] != right[1]:
    ...
    if full(board):
        ...
        if yourscore > cpuscore:
            endsurface = font.render("YOU WIN!!!", True, WHITE)
            screen.blit(endsurface, (1000, 700))
        else:
            endsurface = font.render("YOU LOSE!!!", True, WHITE)
            screen.blit(endsurface, (1000, 700))
        pygame.display.flip()
        ...
        screen.blit(restart_text, (1000, 800))
        pygame.display.flip()
...
# Clean up and exit when the outer loop is terminated
pygame.quit()
sys.exit()
